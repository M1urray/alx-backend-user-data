...
+ [x] 0. **Regex-ing**[filtered_logger.py](filtered_logger.py) contains a function called `filter_datum` that returns the log message obfuscated with the following requirements:
  + Arguments:
...
+ [x] 1. **Log formatter**[filtered_logger.py](filtered_logger.py) contains the following updates:
...
+ [x] 2. **Create logger**[filtered_logger.py](filtered_logger.py) contains
...
+ [x] 3. **Connect to secure database**[filtered_logger.py](filtered_logger.py) contains the following updates:
...
+ [x] 4. **Read and filter data**[filtered_logger.py](filtered_logger.py) contains a `main` function that takes no arguments and returns nothing with the following requirements:
...
+ [x] 5. **Encrypting passwords**[encrypt_password.py](encrypt_password.py) contains a script that meets the following requirements:
  + **INFO**: User *passwords* should **NEVER** be stored in *plain text* in a database.
  + Implement a `hash_password` function that expects one string argument name password and returns a salted, hashed password, which is a byte string.
  + Use the `bcrypt` package to perform the hashing (with `hashpw`).

+ [x] 6. **Check valid password**[app.py](app.py) contains an `is_valid` function that expects 2 arguments and returns a boolean:
  + Arguments:
